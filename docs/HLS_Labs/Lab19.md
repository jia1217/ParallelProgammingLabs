...
We can get the corresponding angles as $/45^。$, 26.57&deg, 14.04&deg, etc. It can be proved that any angle with $0\to90\degree$ can be represented as the sum of these angles, which means with enough $\phi_i$ summed up, we can calculate any $\cos$ and $\sin$ values.  
